...
Proof. {
  unfold prog.
  match goal with |- @refines ?S (?P1;WWhile ?C Do ?Q Done) _ =>
                  set (P := ⟨ fun '(i, n, a, x) '(i', n', a', x') => i < n /\ a' = a /\ x' = x /\
                                    (i < i' <= n /\ (forall k, i <= k < i' -> a k <> x) /\ n = n' \/ i' < n /\ a i' = x /\ n' = i')⟩ : @Stmt S);
  ...
  { apply (refines_trans _ _ _ _ (Seq_right_monotonic_refines _ _ _ _ H)).
    clear H.
    match goal with |- @refines ?S (_;(WWhile ?C Do ?Q Done)) _ => epose proof (wfd_while_refines_if C P _ _) end.
    apply (refines_trans _ _ _ _ (Seq_right_monotonic_refines _ _ _ _ H)).
